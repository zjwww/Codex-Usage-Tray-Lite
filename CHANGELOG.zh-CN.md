...
版本标题以实际 [GitHub Release 历史](https://github.com/zjwww/Codex-Usage-Tray-Lite/releases)为准：已公开发布的版本使用 `x.y.z`，从未公开发布的本地迭代使用 `x.y.z-local`。每次 GitHub 发布前，先依据该历史核对两份更新日志的全部版本标题，并同步更新两份 README。明确获准发布的目标可在准备阶段使用公开标题，但这不代表已经发布。`0.3.1` 是当前发布目标；已核实的此前公开版本为 `0.2.16`、`0.2.18` 和 `0.2.26`。

从 `0.1.1-local` 起，每次用户可见更新都使用新版本号和不可覆盖的独立 ZIP。程序版本字段保持纯数字。从本次发布起，本地包使用 `CodexUsageTrayLite-v<版本>-local-win-x64.zip`；获准发布的 GitHub 包在文档核对后单独生成为 `CodexUsageTrayLite-v<版本>-win-x64.zip`，并保留本地候选包。历史归档保持不变，仅对本次授权的 v0.3.1 本地候选包执行一次性更名，ZIP 字节及哈希不变。

## 0.3.1 - 2026-09-25

### 发布准备

- 区分本地包与 GitHub 正式包文件名，并依据真实 GitHub Release 记录核对历史版本标题。两类包均包含双语使用说明、完整更新日志和发布说明；正式包包含审核后的发布文档。

### 修复

- 移除命令提示符启动 GUI 子系统程序时可能出现在命令输出上方的多余空行。确认并清除未变化的提前提示符后，仅当其正上方完整可见的控制台行仍为空白时，程序才会复用 CMD 生成的该间隔行。
- 保留命令输出下方所需的一个空行及完全相同的恢复提示符。非空、不可读取或位于可见区域之外的上一行绝不会被覆盖；原有提示符文字／光标检查与不注入按键的规则保持不变。
- 新增输出行安全断言，并在真实交互式 CMD 中验证最终布局及恢复提示符后的下一条命令。全部 97 项自动化测试通过。

## 0.3.0-local - 2026-09-25

### 变更

- 将直接交互式命令提示符与 PowerShell 输出优化为常规布局：原位清除 Shell 提前绘制且未变化的提示符，在该行开始输出结果，结果后保留一个空行，再恢复完全相同的可见提示符。
- 清除前会再次检查提示符文字、光标列和所在行。若用户已输入文字、光标移动、提示符不受支持／为多行、输出被重定向或控制台 API 失败，则保留 v0.2.30 的安全换行后备。整个过程不会注入 Enter 或其他按键。
- 新增提示符文字／光标／行位置安全断言，并在真实交互式 CMD 中验证恢复提示符后可继续执行下一条命令。全部 97 项自动化测试通过。

## 0.2.30-local - 2026-09-25

### 修复

- 修复 GUI 子系统 EXE 在交互式命令提示符或 PowerShell 中输出后，末尾看不到新的命令行提示符、必须按 Enter 才恢复的问题。程序现在读取 Shell 已绘制的单行提示符，将命令输出移到其下一行，并在输出完成后恢复相同的可见提示符。
- 恢复过程不会发送 Enter、注入按键、修改 Shell 提示符，也不会把程序改成控制台子系统，因此正常启动托盘及开机启动仍不会闪出控制台。重定向／脚本输出保持不变；命令需要严格排序时仍应正常等待进程结束。
- 新增提示符判定回归覆盖，同时保留内部 `--startup` 托盘启动路径。全部 97 项自动化测试通过。

## 0.2.29-local - 2026-09-25

### 修复

- 保留当前用户“开机启动”注册表项既有的内部 `--startup` 启动参数；新命令行解析器现在会让它进入正常托盘启动流程，不再当作无效参数拒绝。
- 新增回归断言，将 `StartupManager.BuildCommand(...)` 生成的启动参数与解析器行为绑定。全部 97 项自动化测试通过。

## 0.2.28-local - 2026-09-25

### 新增

- 每次刷新成功后新增一条结构化 `Refresh.Success` 日志。单行记录数据源、脱敏邮箱、用户等级、5-Hour／Weekly 剩余量和重置时间、可用用量重置次数、获取时间及刷新耗时；缺失值明确写为 `Unknown` 或 `N/A`。
- 新增仅输出英文的命令行界面：`-help`、`-version`、`-status`、`-refresh`、`-exit`／`-quit`、`-log-path` 和 `-config-path`，并支持 `-h`、`-?`、`-v`、`-logs-path`、`-data-path` 别名。每次只接受一个参数，语法无效时返回退出代码 64。
- 新增轻量级会话内命令信号；`-refresh` 与 `-exit`／`-quit` 控制已运行的托盘实例，不会启动第二次 WebView2／CLI 采集。未找到可响应的实例时返回退出代码 2。

### 变更

- `-status` 只输出上次保存的用量快照，不访问网络；路径命令只输出本地路径。不带参数启动 EXE 时仍保持原有通知区域行为。
- 增加命令解析、已保存状态、隐私、命名命令通道及编译后 EXE 标准输出／错误／退出代码覆盖；全部 97 项自动化测试通过。

## 0.2.27-local - 2026-09-25

### 修复

- 忽略原生空快捷键栏的第二次文字绘制回调，避免主要及次要状态行被重复绘制；Weekly、5-Hour、用量重置及上次更新恢复为预期字重。
- 主菜单与子菜单的实际窗口区域、背景和边框统一使用随 DPI 缩放的圆角几何，消除浅色与深色模式下圆角外露出的矩形凸块。
- 保持原生摘要栏宽度预留、无间隙子菜单定位、键盘导航、现有配色层级不变；Windows 高对比度模式继续使用系统方角绘制。
- 新增单次绘制像素回归、主／子菜单区域与尺寸变化覆盖，以及重复区域生命周期检查。93 项自动化测试全部通过。
