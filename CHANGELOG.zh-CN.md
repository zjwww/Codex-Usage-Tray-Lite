...
- 将“关于”菜单的版本前缀由 `v.` 修正为 `v`；本版显示为 `v0.2.26`。
- 保持较小字号右对齐摘要样式以及 v0.2.24 的全部“更新”菜单行为不变。91 项自动化测试全部通过。

## 0.2.25 - 2026-09-24

### 更新

- 在“关于”菜单右侧的版本摘要前增加 `v.` 前缀；本版显示为 `v.0.2.25`。
- 保持现有较小摘要字号、右对齐方式、子菜单几何、“更新”命令及全部功能行为不变。91 项自动化测试全部通过。

## 0.2.24 - 2026-09-24

### 新增

- 在“工具与帮助”子菜单的“帮助”和“关于”之间新增“更新”。右侧摘要显示 `GitHub`，点击后通过 Windows 默认浏览器打开本项目的 GitHub 最新 Release 页面。
- 为英语、简体中文、繁体中文、日语和韩语补充“更新”菜单文字及页面打开失败提示。

### 更新

- “关于”右侧现在使用菜单现有的较小字号右对齐摘要样式，显示程序当前版本号。
- 新增 HTTPS 链接启动校验、结构化更新页面启动日志、菜单顺序／摘要回归、本地化检查及正式子菜单渲染。91 项自动化测试全部通过。

## 0.2.23 - 2026-09-24

### 修复

- 统一主托盘菜单中账号、用量、重置次数、更新时间及带当前设置摘要条目的左侧文字起点，使其与普通命令行对齐。
- 自绘条目现在保留标准 `ToolStripMenuItem` 文字所使用的 Windows 原生 `TextRenderer` 字形留白，不再比普通命令行向左偏移约 5 像素。
- 保持 v0.2.22 的原生菜单项边界及无间隙子菜单定位；右对齐设置摘要、菜单层级、命令与功能行为均未改变。
- 新增渲染器约束回归，并生成英语／简体中文、浅色／深色、CLI／WebView2 和长邮箱字体缩放的正式 WinForms 渲染。90 项自动化测试全部通过。

## 0.2.22 - 2026-09-24

### 修复

- 移除父托盘菜单与子菜单之间剩余的约 11 像素间隙。
- 移除菜单项级别的水平外边距和内边距；此前这些值会扩大菜单项边界，却不会扩大可见的 `ContextMenuStrip` 窗口。视觉留白仍由渲染器的文字几何与内缩选中背景提供，因此保留 R2 外观但不再影响子菜单坐标。
- 将回归基线加强为真正未经修改的 WinForms 子菜单。普通邮箱与长邮箱布局下，五个带摘要的父项现在都必须匹配其原生边缘行为，包括正常的 1 像素边框重叠。

## 0.2.21 - 2026-09-23

### 部分修复

- 修复子菜单分离的真实原因：带右侧摘要的父菜单项，其自定义首选宽度没有进入 `ToolStripDropDownMenu` 的原生文字宽度计算，导致菜单项实际宽度超过可见的 `ContextMenuStrip`。
- 当前选项摘要现在通过 WinForms 原生次级文字布局指标预留宽度，但仍只由自定义渲染器绘制一次，并继续使用已确认的较小字号和次级颜色。隐藏的多余宽度被消除，原生子菜单定位会从父菜单边缘开始。
- 移除 v0.2.20 中无效的方向覆盖，恢复 WinForms 自身的屏幕碰撞处理及左右展开逻辑。
- 目标机器验证显示大间隙已经缩小，但仍残留约 11 像素。首次回归基线仍包含程序自定义的菜单项水平间距；`0.2.22` 修正该基线并从布局边界中移除剩余间距。

## 0.2.20 - 2026-09-23

### 修复尝试

- 曾尝试在展开前显式选择紧贴父菜单的左／右方向，以解决子菜单分离问题。
- 目标机器验证显示空隙没有变化，因为父菜单项的实际宽度仍超过可见菜单；此尝试已由 `0.2.21` 的尺寸指标修正取代。

## 0.2.19 - 2026-09-23

### 更新

- 优化原生托盘菜单：行高至少为 34 个逻辑像素，并增加适度留白、柔和分隔线、圆角选中高亮，以及适配浅色／深色主题的简洁边框。
- 当前选项摘要改为独立绘制，使用更小字号和次级主题颜色；较长的左侧名称与右侧摘要不再争用同一文字栏。
- 邮箱／等级和用量等主要状态保持高对比，重置次数和更新时间等次级状态保持弱化，真正不可用的操作仍清晰显示为禁用状态。
- 勾选项不再显示方块背景，仅保留清晰勾号；原生键盘导航、子菜单行为、屏幕工作区避让、菜单层级与命令均保持不变。
- 正式 WinForms 渲染检查新增英语／简体中文、浅色／深色、WebView2／CLI、选中行、各子菜单、长邮箱，以及 100%／125%／150%／200% 字体缩放场景。89 项自动化测试全部通过。
